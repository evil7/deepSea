...
### 12.7 关键权衡点（第 1 项已拍板，其余待定）

1. **✅ 已拍板（2026-08-21）：心跳完全走 WS**：
   - 在线状态权威源 = DO 内存态（socket 存活 = online），0 额度。
   - 兜底：WS 断连时写一次 D1 `last_seen`（L1），不再常驻 HTTP 心跳。
   - 配合配额约束（§4.4）：单用户最多 3 节点，WS 常驻连接数有界，无滥用风险。
2. **register 是否并入 WS 首帧**：
   - 并入：登录后仅 WS 1 req（register 作为 WS 首条消息，DO 转写 D1）。
   - 独立：register 1 + WS 1（更清晰，且 register 失败可独立重试；配额校验结果也可独立返回）。
   - **倾向**：独立（清晰优先 + 配额校验在 register 原子返回，1 req 差异可忽略）。
3. **主站是否也常驻 WS**：
   - 常驻：主站也能被推送（如「设备端主动连主站」场景），但多一条连接。
   - 按需：主站仅在「点卡片连接」窗口内建 WS，连接完成即断开。
   - **倾向**：按需（主站侧无需常驻，进一步省连接）。
