...
/** 心跳续期（保持 online 状态；worker 阈值 90s，建议 ≤30s 调用一次）。 */
export async function heartbeatNode(nodeId: string): Promise<boolean> {
  const data = await authFetch<{ ok?: boolean }>("/auth/node/heartbeat", {
    method: "POST",
    body: JSON.stringify({ nodeId }),
  })
  return data?.ok === true
