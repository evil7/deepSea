...
//   前端按「REST API 优先 → raw 保底 → 仓库 /data 冷启动兜底」三级容错读取：
//     · api.github.com/repos/{owner}/{repo}/contents/{file}?ref={branch}
//       带 Accept: application/vnd.github.raw（主路径）
...
/** cache 分支文件的 raw 地址（refs/heads 格式，零 API 配额 + CORS 允许，保底） */
...
/** cache 分支文件的 REST API raw 地址（主路径） */
...
/** 读 cache 分支缓存（REST API 优先，raw 保底；均失败抛错由调用方降级） */
export async function loadCacheFile<T>(file: string): Promise<T> {
  // ① 主路径：REST API contents（Accept: application/vnd.github.raw 直接返回原始内容）
  try {
  ...
  // ② 保底：raw.githubusercontent.com（零 API 配额）
  const res = await fetch(cacheRawUrl(file), { cache: "no-store" })
  if (!res.ok) {
    throw new Error(`cache raw ${res.status}`)
  }
 ...
 *   ① 先读 cache 分支（REST API → raw 两级）；失败则 ② 读仓库 /data 静态 seed；
 *   再失败返回空数组。
 */
