// ---------------------------------------------------------------------------
// /links · /link/:nodeId —— 操作互联（deepc-link 远程控制 · 自实现 chatUI）
//
...
// 数据经 deepc-link 加密 RTC 通道（WebRTC DataChannel）访问本地 dsh host。
// ---------------------------------------------------------------------------
...
import { listNodes, registerConsoleNode, removeNode, isConsoleNode, type NodeView } from "@/lib/deepc-link/nodes"
...
export function LinksPage() {
  ...
  // 在线状态由常驻 WS presence 体现（见下方 useEffect），不再发 HTTP 心跳。
  // URL 携带 /link/:nodeId 时自动连接该节点；否则仅注册。
  useEffect(() => {
    if (!user) {
  ...
  // 登录后常驻 WS（DO 信号房）订阅 presence 广播：node 上线/下线即推，实时刷新设备在线状态。
  // 取代 30s HTTP 心跳——在线判定权威源 = DO 内存态（socket 存活），0 worker 额度。
  useEffect(() => {
    ...
    void ws.connect(consoleNodeId).then((ok) => {
      ...
      off = ws.onPresence((online) => {
        ...
        setNodes((prev) =>
          prev.map((n) => ({ ...n, online: onlineSet.has(n.nodeId) }))
        )
      })
    })
    return () => {
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {nodes.map((node) => (
                <NodeCard
                  ...
                  node={node}
                  onConnect={() => {
                    // 导航到 /link/:nodeId → useEffect 自动触发连接。
                    if (consoleNodeId) {
